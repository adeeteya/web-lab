...
app.post('/submit', function (req, res) {
    ...
    var record =
    {
        "Name": data.name, "CardNo": cardno, "Month": data.month_id, "Year": data.year_i
d, "CVV": data.cvv
    }
...
app.get('/all', function (req, res) {
    dbConn.then(function (db) {
        ...
        dbo.collection("creditcard").find({},
            { "Name": 1, "CardNo": 0, "Month": 0, "Year": 0, "CVV": 0 }).toArray(function (err,
                result) {
                    if (err) throw err;
                console.log(result);
    ...
    dbConn.then(function (db) {
        ...
        dbo.collection("creditcard").deleteMany(myquery, function (err,
            obj) {
            if (err) throw err;
